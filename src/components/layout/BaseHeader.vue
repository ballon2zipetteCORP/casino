<template>
  <header>
    <nav>
      <ul>
        ...
        <li v-if="me?.groups?.includes('/staff')">
          <router-link :to="{ name: 'give' }">
            <mdicon name="account-cog-outline" /> Staff
          </router-link>
        </li>
      </ul>
    </nav>
  ...
  <TokenGetInstructions
    @close="isTokenGetInstructionsShown = false"
    v-show="isTokenGetInstructionsShown"
  />
</template>
...
const formattedZipetteCoins = computed(() => {
  ...
  return `${numbers}<span style="font-size: 1em;color: var(--gray-2);">,${digits.padStart(
    2,
    "0"
  )}</span>`;
});
